...
def callback_waypoints(data):
    ...
    trajectory_points = data.poses
    rospy.loginfo("Received %d trajectory points", len(trajectory_points))

def process_point():
    global current_index, xd, yd, yawd, yawd_prev
    if current_index < len(trajectory_points):
        pose = trajectory_points[current_index].pose
        xd = pose.position.x
        yd = pose.position.y
        ...
        euler = tf.transformations.euler_from_quaternion(quaternion)
        yawd = euler[2]
        rospy.loginfo("Trajectory point: xd=%f, yd=%f, yawd=%f", xd, yd, yawd)
    else:
        rospy.loginfo("All trajectory points have been processed.")
...
def is_approaching_curve(lookahead=15): # To detect when a vehicle is about to approach a curve (threshold of 0.1 rad, i.e. thresoldR variable at the beginning of the controller's code)
    global current_index, trajectory_points, yaw

    if current_index + lookahead < len(trajectory_points):
        next_pose = trajectory_points[current_index + lookahead].pose
...
def ackermann_controller():
    global rho, gamma, delta, tresholdM, tresholdR, k1

    rho = math.sqrt((xd - x)**2 + (yd - y)**2)
    gamma = normalize_angle(math.atan2((yd - y), (xd - x)) - yaw)
    delta = gamma - normalize_angle(yawd - yaw)

    if current_index + 5 > len(trajectory_points): # To slow down the vehicle when it approaches the end of the trajectory
        k1 = 0.7
        tresholdM = 0.25
    else:
    ...
    if is_approaching_curve() or abs(yaw - yawd) > 0.1: 
        tresholdM = 1
        tresholdR = 0.5 # When the vehicle is about to approach a curve or when it is inside one, we have chosen to increase the angular threshold so that points are taken a little further apart from each other to facilitate the maneuver (k1, k2 and k3 higher, as well as v and phi)
...
def plot_results():
    global trajectory_points, trajectory_actual, velocities, steering_angles

    # Reference trajectory
    ref_x = [p.pose.position.x for p in trajectory_points]
    ref_y = [p.pose.position.y for p in trajectory_points]

    # Activate interactive mode
    plt.ion()
    # Actual trajectory
    actual_x, actual_y = zip(*trajectory_actual)
    
    # Plot
    plt.figure(figsize=(10, 10))
    plt.plot(ref_x, ref_y, 'r-', label='Reference Trajectory')
    plt.plot(actual_x, actual_y, 'b', label='Vehicle Trajectory')
...
def main():
    global current_index, trajectory_points, simulation_complete
    ...
    while not rospy.is_shutdown():
        if trajectory_points:
            ...
            if x is not None and y is not None and yaw is not None and xd is not None and yd is not None and yawd is not None:
                while abs(xd - x) > tresholdM or abs(yd - y) > tresholdM or abs(abs(yawd) - abs(yaw)) > tresholdR: # The angular condition with the "abs" function is used to avoid ambiguity when yaw is on the max/min threshold of the angular definition (+pi or -pi)
            ...
            rospy.loginfo("Waiting to receive trajectory points...")
        rate.sleep()
